# tock

[English version](./README.md)

Tock is a powerful time tracking tool for the command line. It saves activity logs as plaintext files and provides an interactive terminal UI for viewing your time.

![tock](https://repo.x-cmd.io/tock.svg?lang=zh)

## 安装

```sh
x install tock
```

## 代码洞察

合计: **15,411** 行代码（覆盖前 5 种语言、共 **122** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Go | 14,912 | 620 | 2,544 | 117 |
| Json | 231 | 0 | 0 | 1 |
| Sh | 171 | 36 | 36 | 2 |
| Python | 62 | 4 | 15 | 1 |
| Makefile | 19 | 2 | 4 | 1 |

## 源代码

- **上游仓库**: <https://github.com/kriuchkov/tock>
- **许可证**: GPL-3.0

## 发布

- **最新版本**: `v2.0.4` (2026-08-02)
- **最近提交**: 2026-08-02
- **Release 含资产**: 5 个

## 流行度

- **Star**: 477 · **Fork**: 21 · **开放 issue**: 33 · **贡献者**: 7

## 累计统计

- **发布数**: 55 · **已合并 PR**: 67 · **开放 PR**: 3 · **已关闭 issue**: 31 · **开放 issue**: 2 · **提交数**: 99

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-13 | 0 | 0 | 2 | 0 | 0 | 0 |
| last60d | 2026-07-14 | 4 | 8 | 3 | 1 | 0 | 8 |
| 90d | 2026-06-14 | 4 | 9 | 3 | 1 | 0 | 9 |
| last180d | 2026-03-16 | 17 | 27 | 3 | 8 | 2 | 31 |
| 360d | 2025-09-17 | 55 | 67 | 3 | 31 | 2 | 99 |
| last720d | 2024-09-22 | 55 | 67 | 3 | 31 | 2 | 99 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [checksums.txt](https://github.com/kriuchkov/tock/releases/download/v2.0.4/checksums.txt) | 364 B | `other` |
| [tock_Darwin_arm64.tar.gz](https://github.com/kriuchkov/tock/releases/download/v2.0.4/tock_Darwin_arm64.tar.gz) | 5.5 MiB | `native/darwin/arm64` |
| [tock_Darwin_x86_64.tar.gz](https://github.com/kriuchkov/tock/releases/download/v2.0.4/tock_Darwin_x86_64.tar.gz) | 5.9 MiB | `native/darwin/x64` |
| [tock_Linux_arm64.tar.gz](https://github.com/kriuchkov/tock/releases/download/v2.0.4/tock_Linux_arm64.tar.gz) | 4.5 MiB | `native/linux/arm64` |
| [tock_Linux_x86_64.tar.gz](https://github.com/kriuchkov/tock/releases/download/v2.0.4/tock_Linux_x86_64.tar.gz) | 5.0 MiB | `native/linux/x64` |

## 发行版状态

在 [repology.org](https://repology.org/project/tock) 上共有 **3** 个发行版报告此项目。**2** 个 ✅ 已是最新上游版本，**0** 个 ⚠️ 使用旧版本。

| 发行版 | 版本 | 状态 |
|--------|------|------|
| Homebrew | `2.0.4` | ✅ latest |

## 改进这些数据

tock 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `tock` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/tock.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260912.yml` · 2026-09-12T04:23:49Z._
